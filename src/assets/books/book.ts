...
export default class Book {
  ...
  render() {
    const bookTemplate = `
          ...
          this.image ? this.image : "images/books/empty.jpg"
        }" alt="book-image" width="212" height="300" />
				<div class="book__info">
					<div class="book__authors">${this.getAuthors()}</div>
					<div class="book__title">${this.title}</div>
					<div class="book__rating">
						<div class="book__rating-averageRating${
              !this.averageRating ? " hidden" : ""
            }">${this.renderStars(this.averageRating)}</div>
						<div class="book__rating-ratingsCount${!this.pageCount ? " hidden" : ""}">${
      this.pageCount ? this.formatViews(this.pageCount) + " review" : ""
    }</div>
					</div>
					<div class="book__description${!this.description ? " hidden" : ""}">${
      this.description
    }</div>
					<div class="book__retailPrice${
            this.saleability != "FOR_SALE" ? " hidden" : ""
          }">${
      this.saleability == "FOR_SALE"

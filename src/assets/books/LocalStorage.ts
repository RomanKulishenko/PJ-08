...
export default class LocalStorage {
  ...
  public setBooks(currentBook: any) {
    ...
    if (bookArr.length === 0) {
      ...
      currBookInfo.insertAdjacentHTML(
        "beforeend",
        new BuyButton(true).render()
      );
  ...
  public showBooksFromLocalStorage(): void {
    ...
    if (bookArr.length > 0) {
      bookArr.forEach((bookFormLS) => {
        ...
        if (bookElement) {
          const buyButtonElement = bookElement.querySelector(
            ".book__buy"
          ) as HTMLElement;
          if (buyButtonElement) {
            buyButtonElement.remove();
            const bookInfoElement = bookElement.querySelector(
              ".book__info"
            ) as HTMLElement;
            bookInfoElement.insertAdjacentHTML(
              "beforeend",
              new BuyButton(true).render()
            );
          }

...
export default class Books {
  ...
  public loadBooks(booksData: Promise<{ items: Array<any> }>) {
    ...
    booksData
      ...
      .then(() => {
        if (!buttonLoadMore) {
          this.booksContainer?.insertAdjacentHTML(
            "beforeend",
            this.addLoadButtonToHTML()
          );
  ...
  public loadDefaultSubject() {
    ...
    const booksDataDefault: Promise<{ items: Array<any> }> =
      this.apiInstance.getDataBooks(url);
  ...
  private loadMoreBooks = () => {
    ...
    const currentSubject = activeLink
      .getAttribute("href")
      ?.substring(1) as string;
    const maxBooksNow: NodeListOf<Element> =
      this.booksContainer.querySelectorAll(".book");
    enum LazyLoad {
      StartIndex = 0,
      MaxResult = +maxBooksNow.length,
      Step = 6,
    }
    const url = this.apiInstance.getURL(
      currentSubject,
      LazyLoad.StartIndex + LazyLoad.Step,
      LazyLoad.MaxResult
    );
    const booksDataLoadMore: Promise<{ items: Array<any> }> =
      this.apiInstance.getDataBooks(url);
      ...
      if (bookHTML) {
        ...
        if (buyButton) {
          buyButton.addEventListener("click", () =>
            new Buy(false, items, book.id).addToLocalStorage()
          );
        } else {
          ...
          if (bookInfo) {
            bookInfo.insertAdjacentHTML(
              "beforeend",
              new Buy(false, items).render()
            );
            const newBuyButton = bookHTML.querySelector(
              ".book__buy"
            ) as HTMLElement;
            if (newBuyButton) {

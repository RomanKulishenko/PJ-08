export default class Navigation {
  ...
  private renderDots(maxSlides: number) {
    ...
    for (let i = 0; i < maxSlides; i++) {
      dotsHTMLs += `<div class="slider__dot slider__nav-element${
        i === 0 ? " active" : ""
      }" data-index="${i}"></div>`;
    }
    return dotsHTMLs;
  ...
  public switchActiveElements(container: HTMLElement, maxSlides: number) {
    const navElements: NodeListOf<Element> = container.querySelectorAll(
      ".slider__nav-element"
    );

    navElements.forEach((navElement) =>
      navElement.addEventListener("click", handleNavigationClick)
    );
    ...
    function handleNavigationClick(event: Event) {
      ...
      const currentElement: HTMLElement = container.querySelector(
        ".active"
      ) as HTMLElement;
      ...
      if (target.classList.contains("slider__arrow--prev")) {
        nextSlideNumber =
          currentIndexInt === 0 ? maxSlides - 1 : currentIndexInt - 1;
      } else if (target.classList.contains("slider__arrow--next")) {
        nextSlideNumber =
          currentIndexInt === maxSlides - 1 ? 0 : currentIndexInt + 1;
      } else {
  ...
  touchSlide(maxSlides: number, container: HTMLElement) {
    ...
    function handleTouchEnd() {
      const slideContainer: HTMLElement = document.querySelector(
  ...
  public toggleActiveElement(
    ...
    container
      .querySelectorAll(`[data-index="${nextSlideNumber}"]`)
      .forEach((element) => {
  ...
  public autoPLaySlider(
    ...
    let timer: number = setInterval(
      () => {
        return isActive
          ? new Navigation().toggleActiveElement(
              startSlide >= maxSlides
